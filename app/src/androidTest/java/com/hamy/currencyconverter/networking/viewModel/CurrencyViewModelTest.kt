...
internal class CurrencyViewModelTest {
    ...
    fun currencyResponseSuccess() {
        ...
        val method: Method =
            viewModel.javaClass.getDeclaredMethod("handleNewsResponse", Response::class.java)
        method.isAccessible = true
        val resource: Resource<CurrencyName> =
            method.invoke(viewModel, response) as Resource<CurrencyName>
        Assert.assertEquals(
            resource.data,
            currencyName
        ) // Add the expected output in second argument according to your needs.
    }
}
